...
class SSA_Normalize(nn.Module):

    def __init__(self, mean, std):
        super(SSA_Normalize, self).__init__()
        self.mean = mean
        self.std = std
...
class SelectedImagenet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        image = Image.open(os.path.join(
            self.imagenet_val_dir, target_name, image_name)) #self.imagenet_val_dir, image_name))
        if image.mode != 'RGB':
...
class ModelTensorDelta(torch.nn.Module):
    def __init__(self, tensor):
        super(ModelTensorDelta, self).__init__()
...
class ModelTensorDeltaLr(torch.nn.Module):
    def __init__(self, tensor):
        super(ModelTensorDeltaLr, self).__init__()
        self.p = torch.nn.Parameter(tensor)
        self.lr=torch.nn.Parameter(torch.ones_like(tensor))

    def forward(self):
        return self.p * self.lr
